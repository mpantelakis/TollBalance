    ...
    const fetchNotSettledData = async () => {
      ...
      try {
        ...
        const debtsResponse = await fetch(
          `http://localhost:9115/api/notsettled/${id}`,
          {
            headers: {
        ...
        const totalResponse = await fetch(
          `http://localhost:9115/api/totalnotsettled/${id}`,
          {
            headers: {
              "x-observatory-auth": token,
            },
        ...
        if (totalResponse.ok) {
          ...
          setError(
            errorData.message || "Failed to fetch total unsettled debts."
          );
        }
      } catch (err) {
  ...
  const settleDebt = async (creditorId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const newTotal = updatedDebts.reduce(
          (acc, debt) => acc + parseFloat(debt.totalOwed),
          0
        );
            ...
            <div key={index} className="debt-card">
              <div className="debt-header">
                <div className="debt-info">
                  <strong>{debt.creditorName}</strong> is owed{" "}
                  <strong>{debt.totalOwed}€</strong>
                </div>
