...
const NotVerified = () => {
    ...
    const fetchNotVerifiedData = async () => {
      try {
        ...
        const paymentsResponse = await fetch(
          `http://localhost:9115/api/notverified/${id}`,
          {
            headers: {
        ...
        const totalResponse = await fetch(
          `http://localhost:9115/api/totalnotverified/${id}`,
          {
            headers: {
              "x-observatory-auth": token,
            },
        ...
        if (totalResponse.ok) {
          ...
          setError(
            errorData.message || "Failed to fetch total unverified payments."
          );
        }
      } catch (err) {
  ...
  const verifyPayment = async (debtorId) => {
    try {
      const token = localStorage.getItem("authToken");
      const { id: creditorId } = JSON.parse(
      ...
      if (response.ok) {
        ...
        const newTotal = updatedPayments.reduce(
          (acc, payment) => acc + parseFloat(payment.totalSettled),
          0
        );
            ...
            <div key={index} className="debt-card">
              <div className="debt-header">
                <div className="debt-info">
                  <strong>{payment.debtorName}</strong> initiated a payment of{" "}
                  <strong>{payment.totalSettled}€</strong>
                </div>
